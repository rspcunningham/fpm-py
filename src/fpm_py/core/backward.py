...
__all__ = ["kvectors_to_image_series", "xyz_to_kxky"]

def xyz_to_kxky(
    x_y_z: tuple[float, float, float],  # LED position in 3D space (x, y, z) in meters
    ...
    Args:
        x_y_z: LED position in 3D space (x, y, z) in meters, with (0, 0, 0) at optical axis
    ...
    Returns:
        Tensor of k-vector in frequency space, shape (2,)
    """
    device = best_device()
    # Extract LED position
    x, y, z = x_y_z
    ...
    theta = math.atan2(radial_distance, z)
    ...
    # k = (2π/λ) * sin(θ)
    k_magnitude = 2 * math.pi * sin_theta / wavelength
    kx = k_magnitude * math.cos(phi)
    ky = k_magnitude * math.sin(phi)
    ...
    # Combine into k-vector
    # Convert to torch tensor directly from scalars
    return torch.tensor([kx_scaled, ky_scaled], dtype=torch.float32, device=device)
...
def kvectors_to_image_series(
    ...
    pupil_radius: int | None = None,  # Pupil radius in pixels (NA-related)
    pupil: torch.Tensor | None = None  # Optional custom pupil function
) -> ImageSeries:
    """
